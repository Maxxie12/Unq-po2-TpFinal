...
class BuscadorNotTest {
	...
	@Test
	void tituloANegar() {
		buscadorNot.setBuscadorANegar(buscadorPorTitulo);
		buscadorPorTitulo.fraseABuscar("Alrededor del mundo");
		assertFalse(buscadorNot.filtrar(proyectosAFiltrar).contains(proyecto1));
	}
	
	@Test
	void sinLaCategoria() {
		buscadorNot.setBuscadorANegar(buscadorIncluyendoCategorias);
		this.categorias.add(categoria4);
	  assertFalse(buscadorNot.filtrar(proyectosAFiltrar).contains(proyecto4));
	}
	
	@Test
	void sinLaCategoriaPeroConElResto() {
		buscadorNot.setBuscadorANegar(buscadorIncluyendoCategorias);
		this.categorias.add(categoria4);
	  assertTrue(buscadorNot.filtrar(proyectosAFiltrar).size()==3);
	}
